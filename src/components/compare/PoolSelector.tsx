...
export function PoolSelector({ selectedIds, onAdd, disabled = false }: PoolSelectorProps) {
  ...
  return (
    <div ref={containerRef} className="relative">
      <div className={cn(
        "flex items-center gap-2 rounded-xl border border-zinc-700 bg-zinc-900 px-3 py-2.5 transition-colors",
      ...
      {isOpen && results.length > 0 && (
        <div className="absolute top-full mt-1.5 left-0 right-0 z-30 rounded-xl border border-zinc-700 bg-zinc-900 shadow-2xl overflow-hidden">
          <div className="py-1">
            {results.map((pool) => (
              <button
                key={pool.pool}
      ...
      {isOpen && query.trim().length >= 2 && results.length === 0 && (
        <div className="absolute top-full mt-1.5 left-0 right-0 z-30 rounded-xl border border-zinc-700 bg-zinc-900 px-4 py-6 text-center shadow-2xl">
